...
def read_html_file(html_file_path):
    ...
    while iteration < length:
        ...
        try:
            ...
            description = ""
        try:
            foundedOn = j.get('foundedOn', {}).get('year', None)
        except AttributeError:
            foundedOn = ""
        try:
            headquarter = j.get('headquarter', {}).get('address').get('city', None)
        except AttributeError:
            headquarter = ""
        try:
            country = j.get('headquarter', {}).get('address').get('country', None)
        except AttributeError:
            country = ""
        try:
            geographicArea = j.get('headquarter', {}).get('address').get('geographicArea', None)
        except AttributeError:
            geographicArea = ""
        try:
            websiteUrl = j.get('websiteUrl', None)
        except AttributeError:
            websiteUrl = ""
        try:
            phone = j.get('phone', {}).get('number', None)
        except AttributeError:
            phone = ""
        try:
            employeeCountRange_start = j.get('employeeCountRange', {}).get('start', None)
        except AttributeError:
            employeeCountRange_start = ""

        try:
            employeeCountRange_end = j.get('employeeCountRange', {}).get('end', None)
        except AttributeError:
            employeeCountRange_end = ""

        try:
            specialities = j.get('specialities', None)
        except AttributeError:
            specialities = ""

        # Create a new row DataFrame
        new_row = {"name": name, "tagline": tagline, "description": description, "websiteUrl": websiteUrl ,
                   "foundedOn": foundedOn, "headquarter": headquarter, "headquarterCountry": country,
                   "geographicArea": geographicArea, "phone": phone, "specialities": specialities,
                   "employeeCountRange_start": employeeCountRange_start, "employeeCountRange_end": employeeCountRange_end}
...
def main():
    ...
    merged_df = get_all_informations('companies')
    merged_df.to_json("companies.json", orient="records", force_ascii=False)
